"""Switch platform for Pilota Casa on/off control."""
...
async def async_setup_entry(
    ...
    """Set up the Pilota Casa switch entity."""
    async_add_entities([PilotaCasaSwitch(config_entry)])


class PilotaCasaSwitch(SwitchEntity, RestoreEntity):
    """Switch entity for Pilota Casa devices."""
    ...
    def __init__(self, entry: ConfigEntry) -> None:
        ...
        self._group: int = entry.data[CONF_GROUP]
        self._channel: int = entry.data[CONF_CHANNEL]
        self._attr_unique_id = entry.entry_id
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, entry.entry_id)},
            manufacturer="Pilota Casa",
            model="Pilota Casa switch",
            name=entry.title,
            sw_version=format_device_summary(
                self._device_id, self._group, self._channel
            ),
        )
