"""Test the Pilota Casa config flow."""
...
async def test_user_flow(
    ...
    assert result["title"] == "Pilota Casa ID 4660 G 1 CH 1"
...
async def test_invalid_device_id(
    ...
    result = await hass.config_entries.flow.async_configure(
        ...
        user_input={
            CONF_TRANSMITTER: TRANSMITTER_ENTITY_ID,
            CONF_DEVICE_ID: MAX_DEVICE_ID + 1,
            CONF_GROUP: 1,
            CONF_CHANNEL: 1,
        },
    )
...
async def test_invalid_group(
    ...
    """Test validation for invalid group."""
    result = await _start_user_flow(hass)
    ...
    assert result["type"] is FlowResultType.FORM
    assert result["step_id"] == "user"
    assert result["errors"] == {CONF_GROUP: "invalid_group"}
    hass.config_entries.flow.async_abort(result["flow_id"])
